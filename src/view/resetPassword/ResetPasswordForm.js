...
import { checkResetPasswordToken, resetPassword } from './ResetPasswordService';
...
export default function ResetPasswordForm(props) {
  ...
  const checkToken = async token => {
    try {
      const data = await checkResetPasswordToken({ token });
  ...
  const handleResetPassword = data => {
    if (data && data.status) {
      switch (data.status) {
        case 200:
          props.history.replace(routes.RESET_PASSWORD_DONE);
          break;
        default:
  ...
  return (
    <div className="wapper-input-reset-password">
