...
import QuantityEditPopup from './components/QuantityEditPopup';
      ...
      items: [
        {
          ...
          day_detail: [
            ...
            },
            {
              ...
              maximum_range: 3,
              prefill_value: 1,
              date: Date.parse('2020-01-02 00:00:00'),
              weekday: 'tue'
            },
            {
              is_available: true,
...
const renderDayColumn = (columnData, dayName) => (
  <div>
    <div>
      {formatDate(columnData[dayName]?.date, 'DD/MM')}
    </div>
    <div>
...
const renderItemQuantity = (item, dayName, actionProvider) => {
  ...
  if (disabled) {
    ...
    content = <QuantityEditPopup value={dayDetail.prefill_value} minValue={0}
                                 onCancel={(value) => actionProvider.onItemQuantityChanged(dayName, item.code, value)}
                                 onSubmit={(newValue) => actionProvider.onItemQuantityChanged(dayName, item.code, newValue)}
                                 onValueChanged={(newValue) => actionProvider.onItemQuantityChanged(dayName, item.code, newValue)}>
      {button}
    </QuantityEditPopup>
  }
  return (
...
const itemColumns = [
  ...
  },
  {
    ...
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'mon', actionProvider)
  },
  {
    title: (columnData) => renderDayColumn(columnData, 'tue'),
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'tue', actionProvider)
  },
  {
    title: (columnData) => renderDayColumn(columnData, 'wed'),
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'wed', actionProvider)
  },
  {
    title: (columnData) => renderDayColumn(columnData, 'thu'),
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'thu', actionProvider)
  },
  {
    title: (columnData) => renderDayColumn(columnData, 'fri'),
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'fri', actionProvider)
  },
  {
    ...
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'sat', actionProvider)
  },
  {
    ...
    align: 'center',
    width: '66px',
    render: (item, actionProvider) => renderItemQuantity(item, 'sun', actionProvider)
  },
];

const CategoryOrderDetail = (props) => {
  const {catId} = props.match.params;
  const {showPrice} = parseQueryString(props.location.search, {parseBooleans: true});
  ...
  const createFormData = (mapFormData) => {
    ...
    Object.keys(mapFormData).forEach(weekday => {
      result.dates.push({
        ...
        items: Object.keys(mapFormData[weekday].items).map((key) => mapFormData[weekday].items[key])
      })
    });
    return result;
  };
    ...
    return {
      ...
      render: (item) => (
        <Paragraph>
          ...
          <FormattedMessage id="IDS_SHIPPING_WARNING" values={{
      ...
      return {
        ...
        render: (item) => (
          <div className="app-flex-container flex-end order-detail-total-cost-label">
            <Tag>
              <FormattedMessage id="IDS_TOTAL_COST_PER_PACKS"
                                values={{
      ...
      <div className="order-detail-container">
          ...
          <Row className="order-detail-table">
            <Col span={24}>
              <AppTable columns={itemColumns}
                        ...
                        itemsKey="items" groupKey="supplier_name"
        ...
        <div className="action-container app-button">
          ...
          <Button type="primary" className="action-button"
                  style={{float: 'right'}} onClick={submitForm}>
